...
def make_predictions_for_stock(stock_code):
    ...
    processed_data = pd.read_csv(os.path.join(app_config.PROCESSED_DATA_DIR, f"{stock_code}_processed_data.csv"))
    latest_data = processed_data.drop(columns=['date', 'stock_code']).iloc[-1]

    rf_model = joblib.load(os.path.join(app_config.MODEL_DIR, f"{stock_code}_rf_model.pkl"))
    xgb_model = joblib.load(os.path.join(app_config.MODEL_DIR, f"{stock_code}_xgb_model.pkl"))
    lstm_model = load_model(os.path.join(app_config.MODEL_DIR, f"{stock_code}_lstm_model.h5"))
...
def generate_all_predictions():
    ...
    predictions = []
    stocks = [f.split('_')[0] for f in os.listdir(app_config.MODEL_DIR) if f.endswith('.pkl')]
    for stock in stocks:
